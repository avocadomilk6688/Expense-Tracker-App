export default class display {
  ...
  static getBudgetMeta() {
    return JSON.parse(
      localStorage.getItem("budget-meta") ||
        '{"currency":"INR","originalAmount":0}',
    );
  }
  ...
  static setBudgetMeta(currency, originalAmount) {
    localStorage.setItem(
      "budget-meta",
      JSON.stringify({ currency, originalAmount }),
    );
  }

  static getAllTags() {
    const isCleanInitialized = localStorage.getItem("expense_app_clean_v3");
    ...
    if (!isCleanInitialized) {
      ...
      localStorage.setItem("expense_app_clean_v3", "true");
      return defaultTags;
    }
